...
export function Chapter({ number, label, title, aside }: { number: string; label: string; title: string; aside?: string }) {
  ...
  return <div ref={chapter} className="scrap-chapter"><motion.span className="scrap-chapter-progress" aria-hidden="true" style={{ scaleX: reduced ? 1 : scrollYProgress }}/><div><span className="scrap-label"><span className="scrap-chapter-number">{number}</span>{label}</span><h2><Handwritten text={title}/></h2></div>{aside && <p className="scrap-hand"><Handwritten text={aside}/></p>}</div>
}
